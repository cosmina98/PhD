...
class WL_edge():
        ...
        def __init__(self,normalise=False,h=0,node_label=None,edge_label=None):
              self.phi=None
        ...
        def compare(self,Gn):
                ...
                edge_label_check=edge_label != None
                height = self.h
                height = int(height)
                new_GN=[]
                ...
                for G in Gn:
                    ...
                    if node_label_check is True:
                        ...
                        #print(G.nodes(data=True))
                    if edge_label_check is True:
                        labels = list(nx.get_edge_attributes(G, edge_label).values())
                        if  isinstance(labels[0], np.ndarray):
                            labels=["".join(map(str, (item.astype(int)))) for item in labels]
                            for i,edge in enumerate(G.edges(data=True)):
                                edge[2][edge_label]=labels[i]
                                #print((node[1][node_label]))
                    ...
                    for j in range(i, len(Gn)):
                        for e1 in Gn[i].edges(data = True):
                            for e2 in Gn[j].edges(data = True):
                                #if edges label, target/source or source/target match increase similarity matrix by one 
